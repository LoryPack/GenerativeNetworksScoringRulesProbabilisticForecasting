...
from src.nn import ConditionalGenerativeModel, DiscardWindowSizeDim, UNet2D
...
with torch.no_grad():
    ...
    if save_plots:
        # we do plots with 5 predictions
        global_projection = False
        n_predictions_for_plots = 5

        # --- plot the absolute values ---
        fig, axes = plt.subplots(ncols=3, nrows=2, figsize=(16, 4.5),
        ...
        p_real = plot_map_ax(realization[:, :, 0], title="Realization", ax=axes[0, 0],
                             global_projection=global_projection, vmax=vmax, vmin=vmin)
        for i in range(n_predictions_for_plots):
            p_pred = plot_map_ax(da_predictions[i][:, :, 0], title=f"Prediction {i + 1}",
        ...
        differences = [da_predictions[i] - realization for i in range(5)]

        fig, axes = plt.subplots(ncols=3, nrows=2, figsize=(16, 4.5),
                                 subplot_kw=dict(projection=ccrs.PlateCarree(), facecolor="gray"))
        # need to find max and min values over all graphs to have coherent colorbars.
        vmax = max([differences[i].max() for i in range(5)])
        vmin = min([differences[i].min() for i in range(5)])
        ...
        differences = [da_predictions[i] - da_prediction_mean for i in range(5)]
